...
1. **查 UA 知识图谱** — 如果目标涉及已知代码库，查 memory 中 `*-knowledge-graph` 记录。图谱提供文件级依赖顺序和教学导览（tour），帮助确定等价类范围和章节顺序。图谱不替代 wiki/rules 的语义文档，两者互补。
2. 列出要学的概念的等价类（正常/边界/陷阱/拓展）
3. 列出前置依赖（没有它学不了当前概念）
4. 如果前置依赖未学 → 先学前置
5. **只准备第一个等价类的教学代码**，不一次性写全套
...
加载两个文件让子 agent 学习：
1. **`scripts/tutorial/00notes/student-profile.md`** — 学生画像，按这个水平出发
2. **`scripts/tutorial/00notes/lessons-learned.md`** — 已积累的教训清单，子 agent 写作业前必须**逐条自查**确保不再犯
3. 按 teach-concept 的等价类方法论听课
4. 不要不懂装懂，诚实标注不确定的地方（"这步我猜的"）
...
**AST 还原验证**（零依赖、端到端）：不确定时，丢出 AST 树图让子 agent 还原 SQL。

```
主 agent: "这是 [概念] 的 AST 树，还原它应该产生的 SQL"
  $ python scripts/pglast_tool.py tree "<目标 SQL>"
  # 把输出中的树图发给子 agent

子 agent: 根据树图还原 SQL → 跟原 SQL 对比
  ✅ 一致 → 理解正确
  ❌ 不一致 → 说明哪个 AST 节点理解错了
```

优点：不依赖任何外部脚本或配置，只需要 `pglast_tool.py tree`。
能稳定复现——不像断言可能写错，AST 树是客观结构，还原出来的 SQL 对不对一比就知道。
...
| AST 树图 | `scripts/pglast_tool.py tree` | 查看 pglast 解析树 |
...
| 已学教训 | `scripts/tutorial/00notes/lessons-learned.md` | 子 agent 写作业前逐条自查的规则清单 |
